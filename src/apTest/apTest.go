...
func AddVirtualNetworkCard(apNum int) {
	...
	for i := 0; i <= ids; i++ {
		...
		for j := 2; j <= times+1; j++ {
			tempIp := ipAddrPre[i] + strconv.Itoa(j) // strconv.Itoa 把数字转为字符串
			...
			log.Println(msg)   // Printf可打印出格式化的字符串,Println不行
			exeSysCommand(msg) // ifconfig WAN:1 172.16.10.2 netmask 255.255.0.0 up 启动网卡  netsh interface ip add address "本地连接" 192.168.1.100 255.255.255.0
			ap := &ApDevData{
				Ip:            tempIp,
				Mac:           "",
...
func StartUdpProcess() {
	...
	defer udpConn.Close() // 关闭/清理代码中所使用的变量
...
func main() {
	...
	AddVirtualNetworkCard(apNum) // 模拟虚拟网卡
	InitWebListen()
	DumpNetInterfaces()
	go StartUdpProcess()
	//Traverse(*apNum)
